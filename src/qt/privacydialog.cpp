                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMRG ought to be enough for anybody." - Bill Gates, 2017
    ui->zMRGpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMRGSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMRG->setEnabled(false);
        ui->pushButtonMintzMRG->setToolTip(tr("zMRG is currently disabled due to maintenance."));

        ui->pushButtonSpendzMRG->setEnabled(false);
        ui->pushButtonSpendzMRG->setToolTip(tr("zMRG is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMRGpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMRG_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMRG is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMRG...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMRG in ") + 
...
void PrivacyDialog::on_pushButtonSpendzMRG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMRG is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMRG
        sendzMRG();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMRG
    sendzMRG();
}

void PrivacyDialog::on_pushButtonZMRGControl_clicked()
{
    ZMRGControlDialog* zMRGControl = new ZMRGControlDialog(this);
    zMRGControl->setModel(walletModel);
    zMRGControl->exec();
}

void PrivacyDialog::setZMRGControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMRGSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMRG()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid MergeCard Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMRGpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMRG is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MRG </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMRG</b>";
    ...
    // use mints from zMRG selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMRGControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMRGControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmrg selector in case it was used
    ZMRGControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMRG Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " MRG, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMRG Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMRG </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMRG "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMRG "));
    ui->labelzMRGAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMRGSyncStatus->setVisible(fShow);
}
