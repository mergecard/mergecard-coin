...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::MRG, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (mergecard.conf).");
            QMessageBox::warning(this, "MergeCard Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#000; background-color:transparent;}\n a, span { font-family: monospace; }\n span.addr {color:#000; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: transparent;}\n td.d0 {font-weight: bold; color:#000;}\n h2, h3 { white-space:nowrap; color:#000;}\n a { color:#000; text-decoration:none; }\n a.nav {color:#000;}\n";
