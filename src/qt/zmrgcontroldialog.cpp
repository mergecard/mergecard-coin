...
#include "ui_zmrgcontroldialog.h"
...
std::list<std::string> ZMRGControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMRGControlDialog::listMints;

ZMRGControlDialog::ZMRGControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMRGControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMRGControlDialog::~ZMRGControlDialog()
{
    delete ui;
}

void ZMRGControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMRGControlDialog::updateList()
{
...
void ZMRGControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMRGControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMRG_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMRGControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMRGControlDialog::GetSelectedMints()
{
...
void ZMRGControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
